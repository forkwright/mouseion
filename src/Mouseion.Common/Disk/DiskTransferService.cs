...
namespace Mouseion.Common.Disk
{
    ...
    public class DiskTransferService : IDiskTransferService
    {
        ...
        private static readonly ILogger Logger = Log.ForContext<DiskTransferService>();

        public DiskTransferService(IDiskProvider diskProvider)
        {
        ...
        private string ResolveRealParentPath(string path)
        {
            ...
            if (parentPath == null || !_diskProvider.FolderExists(parentPath))
            {
                return path;
            }
        ...
        public TransferMode TransferFolder(string sourcePath, string targetPath, TransferMode mode)
        {
            ...
            Logger.Debug("{Mode} Directory [{SourcePath}] > [{TargetPath}]", mode, sourcePath, targetPath);
            ...
            if (mode == TransferMode.Move && sourcePath.PathEquals(targetPath, StringComparison.InvariantCultureIgnoreCase) && _diskProvider.FolderExists(targetPath))
            {
                ...
                Logger.Verbose("Rename Intermediate Directory [{SourcePath}] > [{TempPath}]", sourcePath, tempPath);
                _diskProvider.MoveFolder(sourcePath, tempPath);

                if (!_diskProvider.FolderExists(targetPath))
                {
                    Logger.Verbose("Rename Intermediate Directory [{TempPath}] > [{TargetPath}]", tempPath, targetPath);
                    Logger.Debug("Rename Directory [{SourcePath}] > [{TargetPath}]", sourcePath, targetPath);
                    _diskProvider.MoveFolder(tempPath, targetPath);
                    return mode;
                }

                Logger.Verbose("Rename Intermediate Directory [{TempPath}] > [{SourcePath}]", tempPath, sourcePath);
        ...
        public int MirrorFolder(string sourcePath, string targetPath)
        {
            ...
            Logger.Debug("Mirror Folder [{SourcePath}] > [{TargetPath}]", sourcePath, targetPath);
        ...
        public TransferMode TransferFile(string sourcePath, string targetPath, TransferMode mode, bool overwrite = false)
        {
            ...
            Logger.Debug("{Mode} [{SourcePath}] > [{TargetPath}]", mode, sourcePath, targetPath);
            ...
            if (mode.HasFlag(TransferMode.Move))
            {
                ...
                {
                    Logger.Verbose("On cifs mount. Starting verified copy [{SourcePath}] to [{TargetPath}].", sourcePath, targetPath);
                    TryCopyFileVerified(sourcePath, targetPath, originalSize);
                    Logger.Verbose("Copy successful, deleting source [{SourcePath}].", sourcePath);
                    _diskProvider.DeleteFile(sourcePath);
                    return TransferMode.Move;
                }
        ...
        private void RollbackPartialMove(string sourcePath, string targetPath)
        {
            try
            {
                Logger.Debug("Rolling back incomplete file move [{SourcePath}] to [{TargetPath}].", sourcePath, targetPath);
        ...
        private void RollbackMove(string sourcePath, string targetPath)
        {
            try
            {
                Logger.Debug("Rolling back file move [{SourcePath}] to [{TargetPath}].", sourcePath, targetPath);
        ...
        private void RollbackCopy(string sourcePath, string targetPath)
        {
            try
            {
                Logger.Debug("Rolling back file copy [{SourcePath}] to [{TargetPath}].", sourcePath, targetPath);
        ...
        private bool ShouldIgnore(DirectoryInfo folder)
        {
            ...
            {
                Logger.Verbose("Ignoring folder {FolderFullName}", folder.FullName);
                return true;
            }
