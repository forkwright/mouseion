...
namespace Mouseion.Common.Http
{
    public class HttpRequestBuilder
    {
        ...
        {
            var clone = (HttpRequestBuilder)MemberwiseClone();
        ...
        protected virtual void ApplyFormData(HttpRequest request)
        {
            ...
            var shouldSendAsMultipart = FormData.Any(v => v.ContentType != null || v.FileName != null || (v.ContentData?.Length ?? 0) > 1024);
            ...
            if (shouldSendAsMultipart)
            {
                ...
                using (var writer = new StreamWriter(bodyStream, new UTF8Encoding(false)))
                {
                    foreach (var formData in FormData)
                    {
                        ...
                        if (formData.FileName.IsNotNullOrWhiteSpace())
                        {
                            summary.AppendFormat("\r\n{0}={1} ({2} bytes)", formData.Name, formData.FileName, formData.ContentData?.Length ?? 0);
                        }
                        else
                        {
                            summary.AppendFormat("\r\n{0}={1}", formData.Name, formData.ContentData != null ? Encoding.UTF8.GetString(formData.ContentData) : string.Empty);
                        }
                    }
            ...
            {
                var parameters = FormData.Select(v => string.Format("{0}={1}", v.Name, Uri.EscapeDataString(v.ContentData != null ? Encoding.UTF8.GetString(v.ContentData) : string.Empty)));
        ...
        public virtual HttpRequestBuilder AddQueryParam(string key, object value, bool replace = false)
        {
            ...
            QueryParams.Add(key, value.ToString() ?? string.Empty);
