...
public class MusicQualityParser : IMusicQualityParser
{
    ...
    public MusicQualityParser(ILogger<MusicQualityParser> logger)
    {
        _logger = logger;
    }
    ...
    public QualityModel ParseQuality(string name)
    {
        ...
        if (result.Quality == Quality.MusicUnknown && !ContainsInvalidPathChars(name))
        {
    ...
    private static QualityModel ParseFromExtension(string name, QualityModel result)
    {
        ...
        {
            // Invalid path characters - return unchanged result
        }
